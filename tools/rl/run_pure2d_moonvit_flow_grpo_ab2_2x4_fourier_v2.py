#!/usr/bin/env python3
"""Fail-closed launcher for the official MoonViT+Flow Fourier GRPO line."""
...
TRAINER = REPO / 'tools/rl/grpo_train_pure2d_moonvit_2x4_fourier_full_extrap_v2.py'
CONFIG = REPO / 'configs/rl/pure2d_moonvit_flow_grpo_2x4_fourier_full_extrap_v2.yaml'
...
PREFLIGHT = REPO / (
    'data/outputs/rl/'
    'pure2d_moonvit_flowonly_grpo_ab2_2x4_fourier_full_extrap_'
    'from19000_preflight_v3'
)
FORMAL = REPO / (
    'data/outputs/rl/'
    'pure2d_moonvit_flowonly_grpo_ab2_2x4_fourier_full_extrap_'
...
EXPECTED_TRAINER_SHA256 = (
    '79c0689431b9b7dac128d78c441711e5614db56a72e50d53131242161e52290f'
)
EXPECTED_CONFIG_SHA256 = (
    '345876b0a53b790a692c0d4ff35f96f83a982cfe3b3f1afd5591d87cd9839d13'
)
...
def audit_preflight(output: Path, launch_log: Path) -> dict:
    ...
    if (
        'full two-stage AB2 deployment alignment PASS max_abs=' not in console
        or 'stage_s=[0.0, 0.6667]' not in console
    ):
        raise RuntimeError('full two-stage deployment-alignment PASS evidence is missing')
    ...
    for row in rows:
        ...
        if [round(float(x), 4) for x in row['geom_sigma_px']] != [0.5, 0.4]:
            raise RuntimeError('wrong Fourier sigma schedule')
        if abs(float(row.get('outer_log_count_mean', -1.0)) - 2.0) > 1e-6:
            raise RuntimeError('each rollout must log exactly two outer actions')
    ...
    if (
        fourier.get('profile') != 'm8_sigma050_040_overlap_composite'
        ...
        raise RuntimeError('official Fourier/reward metadata drift')
    ...
    return {
        ...
        'deployment_alignment_pass': True,
    }
...
def main() -> int:
    ...
    manifest = {
        'status': 'STARTED',
        'mode': args.mode,
        'pipeline': 'moonvit_supervised_then_flowonly_fourier_grpo_v3',
        'output': str(output),
        'gpu': args.gpu,
        ...
        'solver_contract': {
            ...
            'rl_action_count': 2,
            'fourier_modes': 8,
            'fourier_sigma_px': [0.5, 0.4],
            'full_extrap_credit_map': [0, 1],
