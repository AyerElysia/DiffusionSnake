"""Five-action MoonViT+Flow GRPO with immutable AB2 2x4 deployment eval.

Training follows the historical five-stage 20/40/60/80/100-percent trajectory
and applies one Fourier normal-field action per outer stage.  Checkpoint
selection and every reported evaluation use the production trajectory: exactly
two outer stages with four AB2 Flow evaluations each.  The offline MoonViT
encoder and learned feature replacer stay frozen while the inherited Flow is
updated.
"""
...
OFFICIAL_FOURIER_ACTION = {
    (8, (0.8000, 0.7000, 0.6000, 0.5000, 0.4000), False): (
        'legacy5_m8_sigma080_070_060_050_040'
    ),
}
...
def main():
    print('=' * 72)
    print('[RL-PURE2D] MoonViT+Flow 5-action Fourier GRPO / AB2 2x4 deploy')
    print('=' * 72)
    ...
    deployment_outer_steps = int(cv('deployment_outer_steps', 2))
    deployment_fractions = [
        float(x) for x in list(cv('deployment_fractions', [0.6667, 1.0]))
    ]
    if len(deployment_fractions) != deployment_outer_steps:
        raise RuntimeError('deployment fraction count does not match outer stages')
    deployment_stage_s_values = []
    deployment_cumulative_s = 0.0
    for frac in deployment_fractions:
        deployment_stage_s_values.append(float(deployment_cumulative_s))
        deployment_cumulative_s = (
            deployment_cumulative_s
            + (1.0 - deployment_cumulative_s) * float(frac)
        )
    deployment_ode_steps = int(cv('deployment_ode_steps', 4))
    ...
    if (
        outer_steps != 5
        or tuple(round(x, 4) for x in fractions) != EXPECTED_TRAIN_FRACTIONS
    ):
        contract_errors.append('five_stage_training_schedule')
    if not bool(getattr(cfg, 'flow_2d_s_conditioning', False)):
        contract_errors.append('stage_progress_conditioning_disabled')
    if tuple(round(x, 4) for x in stage_s_values) != (0.0, 0.2, 0.4, 0.6, 0.8):
        contract_errors.append('five_stage_progress_schedule')
    if ode_steps != 4:
        contract_errors.append('training_ode_steps')
    if (
        deployment_outer_steps != 2
        or tuple(round(x, 4) for x in deployment_fractions)
        != EXPECTED_DEPLOYMENT_FRACTIONS
        or tuple(round(x, 4) for x in deployment_stage_s_values)
        != (0.0, 0.6667)
        or deployment_ode_steps != 4
    ):
        contract_errors.append('immutable_2x4_deployment_schedule')
    ...
    if train_steps not in (2, 10_000) or eval_every != 50 or save_every != 50:
    ...
    if contract_errors:
        raise RuntimeError(
            'Pure2D five-action-train / 2x4-deploy RL contract failed: {}'.format(
                contract_errors
            )
        )
    ...
    with open(log_dir / 'v5_hparams.json', 'w') as f:
        json.dump({
            ...
            'deployment_schedule': {
                'outer_steps': deployment_outer_steps,
                'fractions': deployment_fractions,
                'stage_s_values': deployment_stage_s_values,
                'ode_steps_per_outer_stage': deployment_ode_steps,
                'solver': 'ab2',
                'total_nfe': deployment_outer_steps * deployment_ode_steps,
    ...
    @torch.no_grad()
    def _new_deployment_latents(output):
        return [
            torch.randn_like(output['i_it_py']) * float(initial_noise_scale)
            for _ in deployment_fractions
        ]

    @torch.no_grad()
    def _deterministic_training_rollout(output, flow, initial_latents=None):
    ...
    @torch.no_grad()
    def _deterministic_deployment(output, flow, initial_latents=None):
        """Production 2x4 rollout used for every reported checkpoint metric."""
        if initial_latents is None:
            initial_latents = _new_deployment_latents(output)
        if len(initial_latents) != len(deployment_fractions):
            raise RuntimeError(
                'deployment initial-latent count does not match outer stages'
            )
        current = output['i_it_py'].detach()
        total_disp = torch.zeros_like(current)
        polys = [current.detach()]
        for outer_idx, frac in enumerate(deployment_fractions):
            c_cur = snake_gcn_utils.img_poly_to_can_poly(current)
            action = _outer_action_mean(
                flow,
                output['cnn_feature'],
                ...
                s_value=deployment_stage_s_values[outer_idx],
                latent=initial_latents[outer_idx],
            )
            current = (current + action).detach()
            total_disp = total_disp + action.detach()
    ...
    def _compute_eval():
        ...
        for eval_idx, eb in enumerate(eval_batches):
            ...
            with torch.random.fork_rng(devices=fork_devices):
                ...
                eval_latents = _new_deployment_latents(out)
            det = _deterministic_deployment(
    ...
    def _dump_group_viz(batch, output, rollouts: List[Dict], quality: torch.Tensor,
        ...
        if det_py is None:
            det_ret_for_viz = _deterministic_training_rollout(output, gcn)
        ...
        if deterministic_polys is not None:
            ...
            if step_delta_rows:
                ...
                cv2.putText(step_delta_legend, 'step-delta view: columns compare sampled and deterministic five-stage RL states',
                            (5, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1, cv2.LINE_AA)
        ...
        meta = {
            'step': int(step),
            'type': 'five_stage_fourier_training_action_group',
    ...
    while step < train_steps:
        ...
        if not deployment_alignment_checked:
            with torch.no_grad():
                # First prove that the manual five-stage RL mean is identical
                # to the model's own deterministic iterative solver.
                rng_state = torch.cuda.get_rng_state(i_init.device)
                training_schedule_disp = gcn.sample_disp_iterative(
                    output['cnn_feature'],
                    output['i_it_py'],
                ...
                aligned = _deterministic_training_rollout(
                    output,
                    gcn,
                    initial_latents=matching_latents,
                )
                training_alignment_error = float(
                    (training_schedule_disp - aligned['disp']).abs().max().item()
                )

                # Then independently prove that the evaluation helper is the
                # unchanged production two-stage/four-AB2 trajectory.
                rng_state = torch.cuda.get_rng_state(i_init.device)
                production_disp = gcn.sample_disp_iterative(
                    output['cnn_feature'],
                    output['i_it_py'],
                    snake_gcn_utils.img_poly_to_can_poly(output['i_it_py']),
                    output['py_ind'],
                    num_iter_steps=deployment_outer_steps,
                    fractions=deployment_fractions,
                    ode_steps=deployment_ode_steps,
                )
                torch.cuda.set_rng_state(rng_state, i_init.device)
                deployment_latents = _new_deployment_latents(output)
                deployment_aligned = _deterministic_deployment(
                    output,
                    gcn,
                    initial_latents=deployment_latents,
                )
                deployment_alignment_error = float(
                    (production_disp - deployment_aligned['disp']).abs().max().item()
                )
            if (
                not math.isfinite(training_alignment_error)
                or training_alignment_error > 1e-5
            ):
                raise RuntimeError(
                    'five-stage AB2 training rollout alignment failed '
                    'max_abs={:.9g}'.format(training_alignment_error)
                )
            if (
                not math.isfinite(deployment_alignment_error)
                or deployment_alignment_error > 1e-5
            ):
                raise RuntimeError(
                    'production 2x4 AB2 deployment alignment failed '
                    'max_abs={:.9g}'.format(deployment_alignment_error)
                )
            deployment_alignment_checked = True
            print(
                '[RL-PURE2D] five-stage AB2 training rollout alignment PASS '
                'max_abs={:.9g} stage_s={}'.format(
                    training_alignment_error,
                    [round(float(x), 6) for x in stage_s_values],
                ), flush=True)
            print(
                '[RL-PURE2D] production 2x4 AB2 deployment alignment PASS '
                'max_abs={:.9g} stage_s={}'.format(
                    deployment_alignment_error,
                    [round(float(x), 6) for x in deployment_stage_s_values],
                ), flush=True)
        ...
        det_ret = _deterministic_training_rollout(
