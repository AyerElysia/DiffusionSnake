...
def main():
    ...
    pred_masks_dir = pathlib.Path(ARGS.result_dir) / "pred_masks"
    ...
    with torch.no_grad():
        for volume_number, (volume_id, items) in enumerate(volumes, start=1):
            ...
            for slice_index, dataset_index in items:
                ...
                # Save prediction mask for VerSe evaluation
                volume_pred_dir = pred_masks_dir / volume_id
                volume_pred_dir.mkdir(exist_ok=True)
                slice_idx = dataset.records[dataset_index]["slice_idx"]
                pred_path = volume_pred_dir / f"{volume_id}_x{slice_idx:04d}_pred_mask.png"
                cv2.imwrite(str(pred_path), pred_label_mask.astype(np.uint16))
