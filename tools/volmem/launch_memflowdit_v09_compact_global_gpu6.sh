...
OUT=data/outputs/volmem/verse_memflowdit_v0_9_compact_global_gpu6
LOG=${OUT}/train_2day_gpu6.log
DEADLINE=2026-08-03T23:50:00+08:00
MAX_STEP=2000

mkdir -p "${OUT}"

CUDA_VISIBLE_DEVICES=6 nohup python tools/volmem/train_memflowdit.py \
    --cfg_file "${CFG}" \
    --device cuda:0 \
    --max_steps "${MAX_STEP}" \
    --save_every 100 \
    ...
    --chunks_per_step 8 \
    --chunk_length 12 \
    --init-memflow-ckpt /dev/shm/memflowdit_checkpoints/v08_evidence_value_mm_step_002000.pt \
    </dev/null >"${LOG}" 2>&1 &
TRAIN_PID=$!
