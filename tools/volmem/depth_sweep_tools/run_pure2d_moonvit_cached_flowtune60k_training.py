#!/usr/bin/env python3
"""Fail-closed launcher for cached-MoonViT, trainable-Flow 60k trial."""
...
EXPECTED_CONFIG_SHA256 = "f8095f2754e2c3f0b94c4d7fdedc6b880bca5de40231f2603561ed557db3caaf"
...
def main():
    ...
    if expected_parent not in output_root.parents:
        raise RuntimeError(f"output must stay under {expected_parent}: {output_root}")

    config_sha = sha256_file(config)
    assert_equal(config_sha, EXPECTED_CONFIG_SHA256, "config SHA256")
    ...
    source = Path(str(cfg.resume_path)).resolve()
    cache_root = Path(str(cfg.locate_feat_cache_root))
    if not cache_root.is_absolute():
        cache_root = (worktree / cache_root).resolve()
    if not source.is_file() or not cache_root.is_dir():
        raise FileNotFoundError({"source": str(source), "MoonViT cache": str(cache_root)})
    assert_equal(sha256_file(source), EXPECTED_SOURCE_SHA256, "source checkpoint SHA256")
