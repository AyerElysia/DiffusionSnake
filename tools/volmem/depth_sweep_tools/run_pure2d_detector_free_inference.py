...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        choices=("flow_box_only", "flow_box_cnn"),
        help="fail-closed detector-free feature backend gate",
    )
    parser.add_argument(
        "--expected-backbone",
        default="",
        help="required pure2d_cnn_backbone identity for flow_box_cnn",
    )
    parser.add_argument(
        "--teams-metric-module",
        help="optional TEAMS-style 2D metric adapter; requires --teams-upstream-root",
    )
    parser.add_argument(
        "--teams-upstream-root",
...
def strict_load(wrapper, checkpoint_path):
    ...
    for key, value in raw.items():
        ...
        if key in normalized:
            raise RuntimeError("checkpoint key collision: {}".format(key))
...
def main(argv=None):
    ...
    if (teams_metric_path is None) != (teams_upstream_root is None):
        raise ValueError(
            "--teams-metric-module and --teams-upstream-root must be provided together"
        )
    for path, label in (
        (config_path, "config"),
    ...
    configured_backbone = str(getattr(cfg, "pure2d_cnn_backbone", "") or "")
    if cfg.detector_backend == "flow_box_cnn":
        if not str(args.expected_backbone).strip():
            raise RuntimeError("--expected-backbone is required for flow_box_cnn")
        if configured_backbone != str(args.expected_backbone):
            raise RuntimeError(
                "CNN backbone gate failed: config={} expected={}".format(
                    configured_backbone, args.expected_backbone
                )
            )
    elif str(args.expected_backbone).strip():
        raise RuntimeError("--expected-backbone is only valid for flow_box_cnn")
    ...
    context_module = getattr(base_network, "pure2d_cnn_context", None)
    if cfg.detector_backend == "flow_box_cnn" and context_module is None:
        raise RuntimeError("flow_box_cnn context module was not constructed")
    if cfg.detector_backend == "flow_box_only" and context_module is not None:
        raise RuntimeError("flow_box_only unexpectedly constructed CNN context")
    ...
    with torch.no_grad():
        for batch_index, batch in enumerate(loader):
            ...
            if processed % 160 < len(masks):
                print("[direct-eval] {}/{} slices".format(processed, EXPECTED_SLICES), flush=True)
            del output, batch, masks
    torch.cuda.synchronize(device)
    elapsed = time.perf_counter() - start
    if processed != EXPECTED_SLICES:
    ...
    result = {
        "schema": "diffusionsnake.pure2d_detector_free_inference.v2",
        "status": "PASS",
        "identity": {
        ...
        "model": {
            ...
            "feature_backbone": configured_backbone or None,
            "native_input_resolution": [512, 512],
            "feature_output_resolution": (
                [128, 128] if cfg.detector_backend == "flow_box_cnn" else [128, 128]
            ),
        ...
        "cohort": {
            "case_ids": list(DEV8_CASES),
            "case_count": len(DEV8_CASES),
    ...
    print(json.dumps({
        ...
        "checkpoint_step": checkpoint_step,
        "scan_equal_dice_mean": verse_summary["main"]["scan_equal_dice_mean"],
        "identification_rate_mean": verse_summary["main"]["scan_equal_identification_rate_mean"],
