...
def parse_args():
    ...
    parser.add_argument(
        "--save-viz",
        action="store_true",
        help="Save per-slice GT/prediction error overlays under result-dir/viz.",
    )
    parser.add_argument(
        "--viz-positive-only",
        action="store_true",
        help="Only save slices containing GT foreground.",
    )
...
def _save_visualizations(
    ...
    """Save detector-independent foreground error maps for one prediction chunk."""
    if not ARGS.save_viz:
        return
    output_dir = pathlib.Path(ARGS.result_dir) / "viz"
    output_dir.mkdir(parents=True, exist_ok=True)
    for (slice_index, dataset_index), pred_label_mask in zip(
        item_chunk, pred_label_masks
    ):
        record = dataset.records[dataset_index]
        image = cv2.imread(record["image_path"], cv2.IMREAD_UNCHANGED)
        gt_mask = cv2.imread(record["mask_path"], cv2.IMREAD_UNCHANGED) > 0
        pred_mask = pred_label_mask > 0
        if image is None:
            continue
        if ARGS.viz_positive_only and not bool(gt_mask.any()):
            continue
        ...
        normalized = np.clip(
            (gray.astype(np.float32) - low) / max(float(high - low), 1e-6),
            0.0,
            1.0,
        )
        rgb = cv2.cvtColor(
            (normalized * 255.0).astype(np.uint8), cv2.COLOR_GRAY2BGR
        )
        overlay = (rgb.astype(np.float32) * 0.42).astype(np.uint8)
        true_positive = np.logical_and(gt_mask, pred_mask)
        false_negative = np.logical_and(gt_mask, np.logical_not(pred_mask))
        false_positive = np.logical_and(pred_mask, np.logical_not(gt_mask))
        overlay[true_positive] = (0, 220, 255)  # yellow in BGR
        overlay[false_negative] = (0, 220, 0)   # green: missed GT
        overlay[false_positive] = (0, 0, 255)   # red: excess prediction

        intersection = int(true_positive.sum())
        ...
        cv2.putText(
            panel,
            "{} slice{:04d} Dice={:.3f} IoU={:.3f} | GT box, Memory off".format(
                volume_id, int(slice_index), dice, iou
            ),
            (10, height - 10),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.46,
        ...
        cv2.imwrite(
            str(
                output_dir
                / "viz_{}_slice{:04d}.png".format(volume_id, int(slice_index))
            ),
            panel,
        )
