"""RL V4: three outer inference refinements as the policy trajectory."""
...
def _burr_penalty(final_poly: torch.Tensor, init_poly: torch.Tensor, gt_poly: torch.Tensor,
                  coord_scale: float, margin_px: float, max_px: float) -> tuple[torch.Tensor, torch.Tensor]:
    disp = final_poly - init_poly
    ...
    lap_disp = _contour_laplacian_px(disp, coord_scale).norm(dim=-1)
    excess = torch.relu(lap_final - lap_gt - float(margin_px))
    raw = 0.5 * lap_disp.mean(dim=1) + 0.5 * excess.mean(dim=1)
...
def main():
    print('=' * 72)
    print('[RL-V4] Three outer refinement steps as policy trajectory')
    print('=' * 72)
    ...
    action_std_px = float(cv('action_std_px', 1.5))
    action_std = action_std_px / max(float(snake_config.down_ratio), 1.0)
    ...
    reward_burr_margin_px = float(cv('reward_burr_margin_px', 0.15))
    ...
    for group in optimizer.param_groups:
        group['lr'] = lr
    print(f'[RL-V4] optimizer LR set to {lr}')

    ref_flow = freeze_ref_flow(inner)
    print('[RL-V4] frozen reference flow snapshot created.')
    ...
    with open(log_dir / 'v4_hparams.json', 'w') as f:
        json.dump({
            ...
            'action_std_px': action_std_px,
            'action_std_feature': action_std,
    ...
    def _sample_rollout(output):
        ...
        for frac in fractions:
            ...
            raw_disp = _flow_disp_from_latent(gcn, output['cnn_feature'], current, c_cur, output['py_ind'], latent, ode_steps)
    ...
    def _dump_group_viz(batch, output, rollouts: List[Dict], quality: torch.Tensor,
        ...
        if viz_every <= 0:
            return
        viz_dir = _THIS_DIR / 'visual' / f'rl_v4b_{_cfg_stem()}'
    ...
    for step in range(1, train_steps + 1):
        ...
        for _ in range(k_rollouts):
            ...
            score = _quality_score(final_poly, i_gt, output['image_hw']).detach()
        ...
        for epoch in range(ppo_inner_epochs):
            ...
            for ri, traj in enumerate(rollouts):
                ...
                for si, action in enumerate(traj['actions']):
                    ...
                    lp_cur = _action_logprob(action, mean_cur, action_std)
        ...
        if step % log_every == 0:
            ...
            print(
                ...
                f"burr={metrics['burr_penalty_mean']:.3f} "
        ...
        if viz_every > 0 and (step == 1 or step % viz_every == 0):
            try:
                ...
                print(f'[RL-V4B] viz failed at step {step}: {e}', flush=True)
