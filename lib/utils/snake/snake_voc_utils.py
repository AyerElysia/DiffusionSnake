...
def align_closed_poly(A, B):
    # 方向对齐：顺/逆时针一致
    if signed_area(A) * signed_area(B) < 0:
        A = A[::-1].copy()

    # 起点对齐：让 A 的起点最接近 B[0]
    dists = np.sum((A - B[0])**2, axis=1)
    k = int(np.argmin(dists))
    A = np.roll(A, -k, axis=0)

    # 可选的小优化：在 k 附近微调，取全局距离最小的
    def total_dist(X, Y):
        return np.sum((X - Y)**2)

    candidates = [A, np.roll(A, -1, axis=0), np.roll(A, 1, axis=0)]
    A = min(candidates, key=lambda X: total_dist(X, B))
    return A
