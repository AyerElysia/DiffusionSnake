...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        # #######duan20240720
        # stride = self.stride
        # max_w = max(512, new_unpad[0] + (stride - new_unpad[0]%stride)%stride)
        # max_h = max(512, new_unpad[1] + (stride - new_unpad[1]%stride)%stride)
        # dw, dh = (max_w - new_unpad[0])/2, (max_h - new_unpad[1])/2  # wh padding
        # top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        # left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        # if top+bottom+left+right>0:
        #     img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114))  # add border
        ...
        #     labels["ratio_pad"] = (labels["ratio_pad"], (left, top))  # for evaluation
        # else:
        #     labels["ratio_pad"] = ((new_unpad[0] / shape[1],  new_unpad[1] / shape[0]), (left, top))  # for evaluation
...
class Format:
    ...
    def _format_img(self, img):
        ...
        img = np.ascontiguousarray(img[::-1].copy())  ###duan if random.uniform(0, 1) > self.bgr else img)
