...
class Mamba2(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.in_proj = nn.Linear(self.d_model, int(d_in_proj), bias=bias, **factory_kwargs) #d_model=128, d_in_proj=648
        ...
        self.ce_encoder = nn.Sequential(
            ...
            nn.LayerNorm(64),
            nn.GELU(),
            nn.Linear(64, 8)  # 输出通道数与head数相同
        )

        # 注意力融合层
        self.fusion_attention = nn.MultiheadAttention(
            embed_dim=8,  # 与head数相同
            num_heads=1,
            dropout=0.1,
            batch_first=True
        )
    ...
    def compute_L_matrix(self, A_t):
        """计算掩码L矩阵"""
        B, L, H = A_t.shape # [B, L, H]
        cumprod = torch.cumprod(A_t, dim=1)  # [B, L, H], 在A_t的第一维作累乘
        # Denominator: shift cumprod right
        denominator = torch.ones_like(cumprod)
        denominator[:, 1:, :] = cumprod[:, :-1, :]
        # L_{t1, t2} = prod_{t=t2}^{t1} A_t
        ratio = cumprod.unsqueeze(2) / (denominator.unsqueeze(1) + 1e-8)  # [B, L, L, H]
        # Apply lower-triangular mask
        mask = torch.tril(torch.ones(L, L, device=A_t.device)).unsqueeze(0).unsqueeze(-1)  # [1, L, L, 1]
        L_matrix = ratio * mask  # [B, L, L, H]
        return L_matrix.permute(0, 3, 1, 2)  # [B, H, L, L]
    ...
    def non_casual_linear_attn(self, x, dt, A, B, C, D, H=None, W=None):
        ...
        # 1. 生成转移概率 A_t
        A_t = self.mlp_A(x.reshape(batch*seqlen*head, dim))  # [B*L*8, D=32]
        A_t = A_t.view(batch, seqlen, head)  # [B, L, H]

        L = self.compute_L_matrix(A_t) # [B, 8, L, L]
        Q = C.view(batch, seqlen, self.ngroups, -1).permute(0, 2, 1, 3)  # [B, ngroups, L, dstate_per_group]
        K = B.view(batch, seqlen, self.ngroups, -1).permute(0, 2, 3, 1)  # [B, ngroups, dstate_per_group, L]

        if self.ngroups == 1:
            ## get kv via transpose K and V
            # 计算相似度矩阵
            M = torch.matmul(Q, K)  # [B, 1, dstate, dstate] 原始形状
            # 调整M维度以匹配L
            M = M.permute(0, 1, 3, 2)  # [B, 1, L, L]
            # 应用结构化注意力
            Y = L * M  # [B, H, L, L]
            x = torch.matmul(Y, V)
    ...
    def forward(self, u, H, W, seq_idx=None, poly=None):
        """
        u: (B,C,H,W)
        ...
        if self.linear_attn_duality:
            y, L = self.non_casual_linear_attn(
...
# snack在初始化vm2时传入4个参数：state_dim, out_state_dim, n_adj, dilation
    ...
    def forward(self, x, adj=None, poly=None, H=None, W=None):
        B, L, C = x.shape
