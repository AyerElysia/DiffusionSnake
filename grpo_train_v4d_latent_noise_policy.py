"""RL V4d: three outer refinements with latent noise as the policy action."""
...
class GlobalLatentNoisePolicy(torch.nn.Module):
    """Minimal learnable policy over the initial latent noise of each outer step."""

    def __init__(self, outer_steps: int, init_std: float, min_logstd: float, max_logstd: float):
        super().__init__()
        outer_steps = max(int(outer_steps), 1)
        init_std = max(float(init_std), 1e-6)
        self.mu = torch.nn.Parameter(torch.zeros(outer_steps, 1, 1, 2))
        self.logstd = torch.nn.Parameter(torch.full((outer_steps, 1, 1, 2), math.log(init_std)))
        self.min_logstd = float(min_logstd)
        self.max_logstd = float(max_logstd)

    def forward(self, step_index: int, state: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        idx = max(0, min(int(step_index), self.mu.size(0) - 1))
        mu = self.mu[idx].to(device=state.device, dtype=state.dtype).expand_as(state)
...
def main():
    ...
    noise_policy = GlobalLatentNoisePolicy(
    ...
    print(f'[RL-V4d] optimizer LR set to {policy_lr} | freeze_flow={freeze_flow}')
    ...
    def _sample_rollout(output):
        ...
        for step_idx, frac in enumerate(fractions):
            c_cur = snake_gcn_utils.img_poly_to_can_poly(current)
            mean_z, logstd_z = noise_policy(step_idx, current)
    ...
    def _policy_three_step(output):
        ...
        for step_idx, frac in enumerate(fractions):
            c_cur = snake_gcn_utils.img_poly_to_can_poly(current)
            mean_z, _ = noise_policy(step_idx, current)
            raw_disp = _flow_disp_from_latent(gcn, output['cnn_feature'], current, c_cur, output['py_ind'], mean_z, ode_steps)
            action = raw_disp * float(frac)
            current = (current + action).detach()
            total_disp = total_disp + action.detach()
        return {'disp': total_disp, 'py': output['i_it_py'] + total_disp}
    ...
    for step in range(start_step, train_steps + 1):
        ...
        for epoch in range(ppo_inner_epochs):
            ...
            for ri, traj in enumerate(rollouts):
                ...
                for si, latent in enumerate(traj['latents']):
                    ...
                    mean_cur, logstd_cur = noise_policy(si, state)
                    ...
                    latent_kl = _latent_kl_to_base(mean_cur, logstd_cur, latent_init_std).mean() / max(total_actions, 1)
        ...
        metrics = {
            ...
            'latent_mu_abs': float(noise_policy.mu.detach().abs().mean().item()),
            'latent_std_mean': float(torch.exp(noise_policy.logstd.detach()).mean().item()),
