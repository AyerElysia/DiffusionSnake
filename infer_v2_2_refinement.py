...
def prepare_v2_2_init(batch, dr):
    ...
    ind = torch.zeros((num_valid,), dtype=torch.long, device=gt_all.device)
...
def run_inference(model, device, batch, index, save_dir):
    ...
    save_path = os.path.join(save_dir, f"v2_2_refine_{index}_{datetime.datetime.now().strftime('%H%M%S')}.png")
