...
class Dataset(VocDataset):
    ...
    def _mask_to_instances(mask):
        ...
        for label in np.unique(mask):
            ...
            # Keep only the largest connected component per class label.
            # Segmentation noise can split one vertebra into several tiny
            # fragments; treating each fragment as its own instance both
            # explodes the per-slice instance count (observed up to 51 vs.
            # ~20 real vertebrae, causing cross-attention OOM) and breaks
            # the one-contour-per-class assumption used by prev-frame
            # contour matching at inference time.
