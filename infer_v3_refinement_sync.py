import sys, os
"""
Evolutionary Dynamic Network (V3) Batch Inference Script (Auto-Naming Version)
--------------------------------------------------------------
...
Modified: Added ver_tag for clear naming.
"""
import cv2
import torch
import numpy as np
...
_DEFAULT_CFG = os.path.join(_THIS_DIR, 'configs', 'btcv_diffusion_dit_v3_1.yaml')
...
def load_v3_model():
    ...
    # 动态匹配权重路径
    ckpt_path = getattr(args, 'ckpt', '') or \
               os.path.join(_THIS_DIR, 'data/outputs', cfg.model_dir.split('/')[-1], 'checkpoints', 'latest.pt')
    ...
    if os.path.exists(ckpt_path):
        ...
        print(f"[✔] Weights loaded! Missing: {len(info.missing_keys)}, Unexpected: {len(info.unexpected_keys)}")
        if info.missing_keys: print(f"    - Sample Missing: {info.missing_keys[:3]}")
    else:
...
def run_inference(model, device, batch, index, save_dir):
    ...
    with torch.no_grad():
        ...
        if i_it_py.size(0) > 0:
            ...
            print(f"  [DEBUG] Disp Stats: min={disp.min().item():.4f}, max={disp.max().item():.4f}, mean={disp.abs().mean().item():.4f}")
    ...
    if 'orig_img' in batch:
        ...
        print("  [!] Warning: 'orig_img' missing from batch, using blank background.")
    ...
    if pred_polys is not None:
        print(f"  [DEBUG] Sample Pred Poly (Top 3 pts): {pred_polys[0, :3].tolist()}")
    
    # 自动识别版本标签
    ver_tag = "v3_1_patchify" if getattr(cfg, 'use_dit_v3_1', False) else "v3_0_perceiver"
    
    # 强制文件名包含 "FINAL_FIX" 标识，方便查找
    save_path = os.path.join(save_dir, f"FINAL_FIX_{datetime.datetime.now().strftime('%H%M%S')}_{ver_tag}_idx{index}.png")
...
def main():
    ...
    save_dir = os.path.join(_THIS_DIR, 'visual', 'v3_dynamic_eval')
