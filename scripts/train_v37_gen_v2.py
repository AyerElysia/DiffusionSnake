#!/usr/bin/env python3
"""
V2 of FM training for V3.7-gen denoiser.
Improvements over V1:
  - Longer training (30K+) with constant+cosine LR schedule
  - No spectral loss (pure FM velocity loss)
  - Deterministic evaluation (x_0=0) for stable metrics
  - Multiple t samples per gradient step for better velocity field coverage
  - Optional: multi-step loss (supervise ODE trajectory, not just velocity)

Usage:
    CUDA_VISIBLE_DEVICES=0 FM_NOISE_SCALE=0.3 FM_EPOCHS=30000 FM_SAVE_DIR=data/outputs/v37v2a \
        python -u scripts/train_v37_gen_v2.py
"""
...
def ode_inference(gcn, cnn_feature, i_it_py, c_it_py, py_ind, sampled_feat,
                  steps=10, noise_scale=0.3, deterministic=True):
...
class ConstantCosineScheduler:
    """Constant LR for first `frac` of training, then cosine decay to eta_min."""
    def __init__(self, optimizer, total_epochs, constant_frac=0.7, eta_min=1e-7):
...
def main():
    class Args:
        ...
        epochs = int(os.environ.get('FM_EPOCHS', '30000'))
        lr = float(os.environ.get('FM_LR', '5e-4'))
        eta_min = float(os.environ.get('FM_ETA_MIN', '1e-7'))
        save_dir = os.environ.get('FM_SAVE_DIR', 'data/outputs/v37v2')
        eval_every = int(os.environ.get('FM_EVAL_EVERY', '500'))
        ...
        # V2 improvements
        use_spectral = int(os.environ.get('FM_USE_SPECTRAL', '0'))  # 0=off (default), 1=on
        constant_frac = float(os.environ.get('FM_CONST_FRAC', '0.7'))
        t_per_step = int(os.environ.get('FM_T_PER_STEP', '1'))
        determ_eval = int(os.environ.get('FM_DETERM_EVAL', '1'))  # 1=deterministic
    ...
    print(f"=== V3.7-gen FM Training V2 ===")
    ...
    print(f"  Spectral loss: {'ON' if args.use_spectral else 'OFF'}")
    print(f"  T per step: {args.t_per_step}")
    print(f"  Deterministic eval: {'Yes' if args.determ_eval else 'No'}")
    ...
    # Override delta_reg and laplacian weights
    ...
    spectral_k = gcn._spectral_loss_k if args.use_spectral else 0
    hf_weight = gcn._hf_loss_weight if args.use_spectral else 0

    print(f"\n[*] Starting V2 FM training ({args.epochs} epochs)...\n")
    t_start = time.time()

    for epoch in range(args.epochs):
        denoiser.train()

        # FM forward with optional multiple t per step
        total_fm_loss = 0.0
        total_reg = 0.0

        for _ in range(args.t_per_step):
            ...
            total_reg += L_reg if isinstance(L_reg, torch.Tensor) else torch.tensor(L_reg)

        total_fm_loss /= args.t_per_step
        total_reg /= args.t_per_step
        loss = total_fm_loss + total_reg
        ...
        log_entry = {
            'epoch': epoch,
            'loss': loss.item(),
            'fm_loss': total_fm_loss.item(),
            'reg': total_reg.item() if isinstance(total_reg, torch.Tensor) else float(total_reg),
            'lr': lr
        }
        ...
        if (epoch + 1) % args.eval_every == 0 or epoch == 0:
            ...
            with torch.no_grad():
                if args.determ_eval:
                    # Deterministic: x_0 = 0, single pass
                    ...
                    # Stochastic: 5-sample average
                    all_ious = []
                    for _ in range(5):
                        disp = ode_inference(gcn, cnn_feature, i_it_py, c_it_py, py_ind,
                                             sampled_feat, steps=args.ode_steps,
                                             noise_scale=noise_scale, deterministic=False)
                        ious_i, _ = compute_iou_from_pred(disp, data, dr)
                        all_ious.append(ious_i)
                    ious = [float(np.mean([all_ious[r][c] for r in range(5)])) for c in range(N)]
            ...
            print(f"  Ep {epoch:5d} | loss={loss.item():.4e} fm={total_fm_loss.item():.4e} "
    ...
    # Final deterministic evaluation with multiple step counts
    denoiser.eval()
    print(f"\n=== Final Evaluation ===")
    with torch.no_grad():
        for steps in [5, 10, 20]:
            ...
            print(f"  Deterministic {steps:3d} steps: IoU={mean_iou*100:.2f}% | {per_str}")

        # Also do stochastic 20-sample
        all_ious = []
        for _ in range(20):
            disp = ode_inference(gcn, cnn_feature, i_it_py, c_it_py, py_ind,
        ...
        mean_iou = float(np.mean(ious))
        per_str = ' '.join([f'C{i}={v*100:.1f}%' for i, v in enumerate(ious)])
        print(f"  Stochastic 20-avg 10 steps: IoU={mean_iou*100:.2f}% | {per_str}")
