#!/usr/bin/env python3
"""
V5 of FM training for V3.7-gen denoiser.
Critical improvement: progressive feature re-sampling at current polygon position.

Diagnosis from V4: C4 (large displacement) stuck at ~60% IoU because features
sampled at initial octagon don't contain boundary information for distant boundaries.

Key changes from V4:
  1. PROGRESSIVE FEATURE RE-SAMPLING: During both training and ODE inference,
     features are sampled at the CURRENT polygon position (i_it_py + x_t), not
     just the initial octagon position. This gives the model progressively better
     boundary information as the polygon converges.
  2. RAW MODE: inherited from V4 (no normalize/denormalize)
  3. Correct IoU: inherited from V4
  4. Concat mode: sampled_feat = cat(init_feat, current_feat) so model has both

Usage:
    CUDA_VISIBLE_DEVICES=0 FM_EPOCHS=15000 FM_SAVE_DIR=data/outputs/v37v5a \
        python -u scripts/train_v37_gen_v5.py
"""
...
def ode_inference_raw(denoiser, cnn_feature, i_it_py, c_it_py, py_ind,
                       init_feat, h, w, steps=10, noise_scale=1.0,
                       deterministic=True):
    """ODE inference in RAW displacement space with progressive feature re-sampling."""
    ...
    for i in range(steps):
        ...
        # Re-sample features at current polygon position
        current_poly = i_it_py + x_t
        current_feat = snake_gcn_utils.get_gcn_feature(cnn_feature, current_poly, py_ind, h, w)
        v_pred, _ = raw_predict_velocity(
            denoiser, cnn_feature, i_it_py, c_it_py, current_feat,
            py_ind, x_t, t_tensor
        )
        x_t = x_t + v_pred * dt
    return x_t
...
def raw_predict_velocity(denoiser, cnn_feature, i_it_py, c_it_py, sampled_feat,
                          py_ind, x_t, t_tensor):
    """Predict velocity using denoiser in RAW space."""
    from lib.utils.snake import snake_gcn_utils, snake_config
...
def main():
    class Args:
        ...
        save_dir = os.environ.get('FM_SAVE_DIR', 'data/outputs/v37v4')
    ...
    print(f"=== V3.7-gen FM Training V5 (RAW + Feature Re-sampling) ===")
    ...
    print(f"  Mode: RAW + progressive feature re-sampling at current poly")
    ...
    x1 = data['x1_raw']  # RAW target displacement
    N = x1.size(0)
    # Auto noise scale: 50% of data std (enough to add diversity, not too much)
    noise_scale = float(x1.std()) * 0.5
    print(f"  Auto noise scale: {noise_scale:.4f}")
    ...
    py_ind = data['py_ind']
    init_feat = data['sampled_feat']  # features at initial octagon (cached)
    h_feat, w_feat = data['h'], data['w']

    # Compute initial octagon IoU for reference
    init_ious, init_mean = compute_true_iou(torch.zeros_like(x1), data, dr)
    ...
    print(f"\n[*] Starting V5 RAW FM training with feature re-sampling ({args.epochs} epochs)...\n")
    t_start = time.time()

    for epoch in range(args.epochs):
        denoiser.train()

        # --- FM velocity loss in RAW space ---
        ...
        # Re-sample features at current polygon position (i_it_py + x_t)
        current_poly = i_it_py + x_t
        current_feat = snake_gcn_utils.get_gcn_feature(cnn_feature, current_poly, py_ind, h_feat, w_feat)
        ...
        if (epoch + 1) % args.eval_every == 0 or epoch == 0:
            denoiser.eval()
            with torch.no_grad():
                # Eval current model (deterministic ODE from x0=0 with feature re-sampling)
                disp = ode_inference_raw(denoiser, cnn_feature, i_it_py, c_it_py,
                                          py_ind, init_feat, h_feat, w_feat,
    ...
    with torch.no_grad():
        for label, use_ema in [("Current", False), ("EMA", True)]:
            ...
            for steps in [5, 10, 20]:
                ...
                rmse = (disp - x1).pow(2).mean().sqrt().item()
