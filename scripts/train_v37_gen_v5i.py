#!/usr/bin/env python3
"""
V5i: Simple Low-LR Control — Same as V5f but with LR=1e-4 (5× lower).

Control experiment for V5h. Tests whether simply reducing LR is sufficient to 
tame oscillation, or if multi-t averaging is needed.

Usage:
    CUDA_VISIBLE_DEVICES=2 FM_EPOCHS=15000 FM_SAVE_DIR=data/outputs/v37v5i_lowlr \
        python -u scripts/train_v37_gen_v5i.py
"""
...
def main():
    class Args:
        ...
        lr = float(os.environ.get('FM_LR', '1e-4'))
        eta_min = float(os.environ.get('FM_ETA_MIN', '1e-7'))
        save_dir = os.environ.get('FM_SAVE_DIR', 'data/outputs/v37v5i_lowlr')
    ...
    print(f"=== V3.7-gen V5i (Low-LR Control: LR=1e-4) ===")
    ...
    print(f"\n[*] Starting V5i low-LR FM training ({args.epochs} epochs)...\n")
    t_start = time.time()
