#!/usr/bin/env python3
"""
V5e of FM training for V3.7-gen denoiser.
Direct regression + per-contour normalization + PerPointFinalLayer.

Key insight from V5b/V5c/V5d: RegularizedPerPointFinalLayer's shared component
causes conflicting gradients between different contours. The shared_linear learns
a compromise that's suboptimal for all contours (especially C4).

V5e uses pure PerPointFinalLayer (128 independent Linear(256→2)) to completely
decouple contour predictions. Combined with:
  1. Per-contour normalization (balanced gradient scales)
  2. Direct regression (stable, no ODE complexity)
  3. FM auxiliary loss at random t (for better feature learning)

This is the "unrestricted" version — max capacity for single-sample.
For generalization, we'll add regularization later if this proves the approach works.

Usage:
    CUDA_VISIBLE_DEVICES=0 FM_EPOCHS=15000 FM_SAVE_DIR=data/outputs/v37v5e \
        python -u scripts/train_v37_gen_v5e.py
"""
...
def main():
    class Args:
        ...
        save_dir = os.environ.get('FM_SAVE_DIR', 'data/outputs/v37v5e')
        ...
        fm_aux_weight = float(os.environ.get('FM_AUX_WEIGHT', '0.1'))  # FM auxiliary loss weight
    args = Args()
    ...
    # Override: use PerPointFinalLayer instead of RegularizedPerPointFinalLayer
    cfg.v3_7_use_regularized_per_point = False
    os.makedirs(os.path.join(args.save_dir, 'checkpoints'), exist_ok=True)

    print(f"=== V3.7-gen FM Training V5e (Direct + PerPointFinalLayer) ===")
    ...
    print(f"  Mode: Direct regression + FM aux + Per-contour norm")
    ...
    # Verify we got PerPointFinalLayer
    ...
    # PerPointFinalLayer has no delta_scale or reg_weight
    ...
    print(f"\n[*] Starting V5e training ({args.epochs} epochs)...\n")
    t_start = time.time()

    for epoch in range(args.epochs):
        denoiser.train()

        # --- Primary: Direct regression at t=0 ---
        v_pred, _ = raw_predict_velocity(
            denoiser, cnn_feature, i_it_py, c_it_py, sampled_feat,
            py_ind, x0_zero, t_zero
        )
        direct_loss = F.mse_loss(v_pred, x1_norm)

        # --- Auxiliary: FM loss at random t (helps DiT learn better features) ---
        if args.fm_aux_weight > 0:
            ...
            loss = direct_loss + args.fm_aux_weight * fm_loss
        else:
        ...
        if (epoch + 1) % args.eval_every == 0 or epoch == 0:
            ...
            if mean_iou > best_iou:
                ...
                torch.save({
                    ...
                    'raw_mode': True, 'contour_norm': True, 'direct_regression': True, 'per_point_head': True,
            ...
            if mean_iou_ema > best_ema_iou:
                ...
                torch.save({
                    ...
                    'direct_regression': True, 'per_point_head': True,
            ...
            print(f"  Ep {epoch:5d} | loss={loss.item():.4e} direct={direct_loss.item():.4e} "
