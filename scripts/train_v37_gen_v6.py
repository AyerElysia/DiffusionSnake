#!/usr/bin/env python3
"""
V6: Fine-tune V5f's best checkpoint with contour-balanced loss.

V5f achieved 91.51% overall IoU but C1 collapsed to 72.6%. The other 5 contours
are 90-98%. This script fine-tunes V5f's checkpoint with:

  1. Per-contour weighted FM loss — higher weight for low-IoU contours
  2. Multi-t K=3 gradient accumulation for stability
  3. Moderate LR with fast cosine decay

Goal: Bring C1 from 72.6% to 90%+ without losing other contours.
Expected final IoU: 93-95%.

Usage:
    CUDA_VISIBLE_DEVICES=2 FM_SAVE_DIR=data/outputs/v37v6_balanced \
        python -u scripts/train_v37_gen_v6.py
"""
...
def main():
    class Args:
        ...
        epochs = int(os.environ.get('FM_EPOCHS', '5000'))
        lr = float(os.environ.get('FM_LR', '2e-4'))
        eta_min = float(os.environ.get('FM_ETA_MIN', '1e-7'))
        save_dir = os.environ.get('FM_SAVE_DIR', 'data/outputs/v37v6_balanced')
        ...
        # Default: load V5f best checkpoint
        ckpt = os.environ.get('CKPT', 'data/outputs/v37v5f_pph_fm/checkpoints/best.pt')
        constant_frac = float(os.environ.get('FM_CONST_FRAC', '0.3'))
        ...
        multi_t_K = int(os.environ.get('FM_MULTI_T_K', '3'))
        # Contour balancing
        balance_temp = float(os.environ.get('FM_BALANCE_TEMP', '0.5'))
        rebalance_every = int(os.environ.get('FM_REBALANCE_EVERY', '200'))
    args = Args()
    ...
    print(f"=== V6: Fine-tune V5f with Contour-Balanced Loss ===")
    ...
    print(f"  Multi-t K: {args.multi_t_K}")
    print(f"  Balance temperature: {args.balance_temp}")
    print(f"  Rebalance every: {args.rebalance_every} epochs")
    ...
    if args.ckpt and os.path.exists(args.ckpt):
        ...
        print(f"  WARNING: No checkpoint found at {args.ckpt}, training from scratch!")
    ...
    # Initial evaluation to get per-contour IoU for weighting
    denoiser.eval()
    with torch.no_grad():
    ...
    print(f"  Initial IoU (from ckpt): {init_mean*100:.2f}%")
    per_str = ' '.join([f'C{i}={v*100:.1f}%' for i, v in enumerate(init_ious)])
    print(f"  Per-contour: {per_str}")

    # Initialize contour weights from IoU: higher weight for lower IoU
    contour_ious = torch.tensor(init_ious, device=device, dtype=torch.float32)
    contour_weights = compute_contour_weights(contour_ious, args.balance_temp)
    print(f"  Initial contour weights: {contour_weights.tolist()}")
    ...
    print(f"\n[*] Starting V6 fine-tuning ({args.epochs} epochs, K={args.multi_t_K})...\n")
    t_start = time.time()
    ...
    for epoch in range(args.epochs):
        ...
        # Multi-t gradient accumulation
        total_fm_loss = 0.0
        for k_step in range(args.multi_t_K):
            ...
            # Per-contour weighted loss
            per_contour_loss = F.mse_loss(v_pred, v_target, reduction='none').mean(dim=(1, 2))  # (N,)
            weighted_loss = (per_contour_loss * contour_weights).mean()
            (weighted_loss / args.multi_t_K).backward()
            total_fm_loss += weighted_loss.item()

        torch.nn.utils.clip_grad_norm_(train_params, 1.0)
        optimizer.step()
        scheduler.step()

        fm_loss_avg = total_fm_loss / args.multi_t_K
        ...
        log_entry = {
            ...
            'fm_loss': fm_loss_avg,
            'lr': lr,
            'contour_weights': contour_weights.tolist()
        }

        # Periodic evaluation + rebalancing
        if (epoch + 1) % args.eval_every == 0 or epoch == 0:
            ...
            if mean_iou_ema > best_ema_iou:
                ...
                torch.save({
                    ...
                    'ema': True, 'raw_mode': True, 'contour_norm': True, 'per_point_head': True,
                }, os.path.join(args.save_dir, 'checkpoints', 'best_ema.pt'))

            # Rebalance contour weights based on current IoU
            if (epoch + 1) % args.rebalance_every == 0:
                contour_ious = torch.tensor(ious, device=device, dtype=torch.float32)
                contour_weights = compute_contour_weights(contour_ious, args.balance_temp)

            elapsed = time.time() - t_start
            per_iou_str = ' '.join([f'C{i}={v*100:.1f}%' for i, v in enumerate(ious)])
            weight_str = ' '.join([f'{w:.2f}' for w in contour_weights.tolist()])
            disp_rmse = (disp - x1_raw).pow(2).mean().sqrt().item()
            print(f"  Ep {epoch:5d} | fm={fm_loss_avg:.4e} | lr={lr:.2e} | "
                  f"IoU={mean_iou*100:.2f}% EMA={mean_iou_ema*100:.2f}% "
                  f"(best={best_iou*100:.2f}%@{best_epoch} ema={best_ema_iou*100:.2f}%@{best_ema_epoch}) "
                  f"| {per_iou_str} | w=[{weight_str}] | {elapsed:.0f}s")
    ...
    print(f"  Initial IoU (V5f ckpt) = {init_mean*100:.2f}%")


def compute_contour_weights(contour_ious, temperature=0.5):
    """Compute per-contour weights: higher weight for lower IoU.
    
    Uses softmax over (1 - IoU) / temperature, then scales so mean = 1.
    """
    error = 1.0 - contour_ious  # Higher error = lower IoU
    N = len(error)
    # Softmax-based weighting
    weights = F.softmax(error / temperature, dim=0) * N
