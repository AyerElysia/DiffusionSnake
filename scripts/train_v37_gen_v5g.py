#!/usr/bin/env python3
"""
V5g: Multi-step ODE training with feature re-sampling.

Key insight: All previous versions (V4-V5f) train with features sampled at the 
initial OCTAGON position. For C4 (largest contour), the octagon is far from the 
true boundary, so features are uninformative → C4 stuck at ~66%.

V5g solves this with trajectory training:
  1. K=3 ODE steps per training epoch
  2. At each step, re-sample features at the CURRENT predicted polygon position
  3. Loss = MSE(final_position, target) — train the trajectory end-to-end
  4. Gradients flow through all K steps (including grid_sample)

After step 0's partial prediction moves C4's contour closer to the boundary,
step 1 gets better features → better prediction → step 2 refines further.

Also includes:
  - PerPointFinalLayer (128 independent heads)
  - Per-contour normalization
  - FM auxiliary velocity loss (regularize velocity field)

Usage:
    CUDA_VISIBLE_DEVICES=1 FM_EPOCHS=15000 FM_SAVE_DIR=data/outputs/v37v5g_ode_resample \
        python -u scripts/train_v37_gen_v5g.py
"""
...
def resample_features(cnn_feature, current_poly, py_ind, h, w):
    """Re-sample CNN features at current polygon positions (differentiable)."""
    from lib.utils.snake import snake_gcn_utils
    return snake_gcn_utils.get_gcn_feature(cnn_feature, current_poly, py_ind, h, w)


def ode_forward_k_steps(denoiser, cnn_feature, i_it_py, py_ind, 
                         contour_scale, h, w, K=3, x0=None):
    """K-step ODE forward with feature re-sampling at each step.
    Returns final normalized displacement and intermediate velocities."""
    from lib.utils.snake import snake_gcn_utils, snake_config
    device = i_it_py.device
    N, P = i_it_py.size(0), i_it_py.size(1)
    adj = snake_gcn_utils.get_adj_ind(snake_config.adj_num, P, device)
    
    if x0 is None:
        x_t = torch.zeros(N, P, 2, device=device)
    else:
        x_t = x0.clone()
    ...
    for k in range(K):
        ...
        # Compute current polygon in feature space
        current_poly = i_it_py + x_t * contour_scale
        
        # Re-sample features at current position (differentiable)
        sampled_feat_k = resample_features(cnn_feature, current_poly, py_ind, h, w)
        
        # Predict velocity
        v_pred, _ = denoiser(cnn_feature, sampled_feat_k, x_t, t_scaled, adj,
                              polys=current_poly, py_ind=py_ind)
        
        velocities.append(v_pred)
        x_t = x_t + v_pred * dt
    
    return x_t, velocities


def ode_inference_resample(denoiser, cnn_feature, i_it_py, py_ind,
                            contour_scale, h, w, steps=10):
    """ODE inference with feature re-sampling at each step."""
    from lib.utils.snake import snake_gcn_utils, snake_config
    device = i_it_py.device
    N, P = i_it_py.size(0), i_it_py.size(1)
    adj = snake_gcn_utils.get_adj_ind(snake_config.adj_num, P, device)
...
def raw_predict_velocity(denoiser, cnn_feature, i_it_py, c_it_py, sampled_feat,
                          py_ind, x_t, t_tensor):
    """Single-step velocity prediction with cached features (for FM aux loss)."""
    from lib.utils.snake import snake_gcn_utils, snake_config
...
def main():
    class Args:
        ...
        save_dir = os.environ.get('FM_SAVE_DIR', 'data/outputs/v37v5g_ode_resample')
        eval_every = int(os.environ.get('FM_EVAL_EVERY', '200'))
        save_every = int(os.environ.get('FM_SAVE_EVERY', '5000'))
        ode_steps_train = int(os.environ.get('FM_ODE_TRAIN_STEPS', '3'))
        ode_steps_eval = int(os.environ.get('FM_ODE_EVAL_STEPS', '10'))
        ...
        fm_aux_weight = float(os.environ.get('FM_AUX_WEIGHT', '0.1'))
    args = Args()
    ...
    print(f"=== V3.7-gen V5g (Multi-Step ODE + Feature Re-sampling) ===")
    print(f"  Config: {args.cfg}")
    print(f"  LR: {args.lr} (constant {args.constant_frac*100:.0f}% → cosine → {args.eta_min})")
    print(f"  Epochs: {args.epochs}")
    print(f"  ODE steps train: {args.ode_steps_train} | eval: {args.ode_steps_eval}")
    print(f"  FM aux weight: {args.fm_aux_weight}")
    print(f"  EMA decay: {args.ema_decay}")
    print(f"  Scope: {args.scope}")
    print(f"  FinalLayer: PerPointFinalLayer (128 independent heads)")
    print(f"  Mode: Trajectory loss + feature re-sampling at each ODE step")
    ...
    K = args.ode_steps_train

    print(f"\n[*] Starting V5g multi-step ODE training ({args.epochs} epochs, K={K})...\n")
    t_start = time.time()

    for epoch in range(args.epochs):
        denoiser.train()

        # --- Primary loss: K-step ODE trajectory with feature re-sampling ---
        x_final, velocities = ode_forward_k_steps(
            denoiser, cnn_feature, i_it_py, py_ind,
            contour_scale, h_feat, w_feat, K=K
        )
        # x_final is in normalized space; target is x1_norm
        traj_loss = F.mse_loss(x_final, x1_norm)

        # --- Auxiliary: FM velocity loss at random t (regularizes velocity field) ---
        if args.fm_aux_weight > 0:
            t_rand = torch.rand(N, device=device)
            t_rand_3d = t_rand.view(N, 1, 1)
            x0_rand = torch.randn_like(x1_norm) * noise_scale
            x_t = (1.0 - t_rand_3d) * x0_rand + t_rand_3d * x1_norm
            v_target = x1_norm - x0_rand
            v_fm, _ = raw_predict_velocity(
                denoiser, cnn_feature, i_it_py, c_it_py, sampled_feat,
                py_ind, x_t, t_rand
            )
            fm_loss = F.mse_loss(v_fm, v_target)
            loss = traj_loss + args.fm_aux_weight * fm_loss
        else:
        ...
        log_entry = {
            ...
            'fm_loss': fm_loss.item() if isinstance(fm_loss, torch.Tensor) else 0.0,
            'lr': lr
        }
        ...
        if (epoch + 1) % args.eval_every == 0 or epoch == 0:
            denoiser.eval()
            with torch.no_grad():
                # Eval with feature re-sampling ODE
                disp = ode_inference_resample(
                    denoiser, cnn_feature, i_it_py, py_ind,
                    contour_scale, h_feat, w_feat, steps=args.ode_steps_eval)
                ...
                disp_ema = ode_inference_resample(
                    denoiser, cnn_feature, i_it_py, py_ind,
                    contour_scale, h_feat, w_feat, steps=args.ode_steps_eval)
            ...
            log_entry['ema_per_iou'] = ious_ema
            ...
            if mean_iou_ema > best_ema_iou:
                ...
                torch.save({
                    ...
                    'per_point_head': True, 'ode_resample': True,
            ...
            print(f"  Ep {epoch:5d} | loss={loss.item():.4e} traj={traj_loss.item():.4e} "
                  f"fm={fm_loss.item():.4e} | lr={lr:.2e} | "
    ...
    with torch.no_grad():
        for label, use_ema in [("Current", False), ("EMA", True)]:
            ...
            for steps in [3, 5, 10, 20]:
                disp = ode_inference_resample(
                    denoiser, cnn_feature, i_it_py, py_ind,
                    contour_scale, h_feat, w_feat, steps=steps)
                ious, mean_iou = compute_true_iou(disp, data, dr)
