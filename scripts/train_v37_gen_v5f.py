#!/usr/bin/env python3
"""
V5f: PerPointFinalLayer + Flow Matching + Per-Contour Normalization.

Key hypothesis: V5c oscillated because RegularizedPerPointFinalLayer's shared 
component caused conflicting gradients between contours. V5e (direct regression +
PerPointFinalLayer) proved stable but got stuck at 81% local minimum.

V5f combines:
  - PerPointFinalLayer (128 independent heads) — no contour interference at output
  - FM training (ODE-based) — better optimization landscape than direct regression
  - Per-contour normalization — balanced gradient scales across contours
  - Beta(0.3, 1.0) t-sampling — bias toward low t

If this eliminates oscillation while achieving >85% IoU, it proves the oscillation
was caused by RegularizedPerPointFinalLayer, not by FM itself.

Usage:
    CUDA_VISIBLE_DEVICES=1 FM_EPOCHS=15000 FM_SAVE_DIR=data/outputs/v37v5f_pph_fm \
        python -u scripts/train_v37_gen_v5f.py
"""
...
def main():
    class Args:
        ...
        save_dir = os.environ.get('FM_SAVE_DIR', 'data/outputs/v37v5f_pph_fm')
    ...
    print(f"=== V3.7-gen V5f (PerPointFinalLayer + FM + Per-Contour Norm) ===")
    ...
    print(f"\n[*] Starting V5f FM+PerPointFinalLayer training ({args.epochs} epochs)...\n")
    t_start = time.time()
    ...
    for epoch in range(args.epochs):
        ...
        # Optional laplacian smoothness loss
        if args.lap_weight > 0:
            pred_shifted = torch.roll(v_pred, 1, dims=1) + torch.roll(v_pred, -1, dims=1)
            lap = v_pred - 0.5 * pred_shifted
            lap_loss = lap.pow(2).mean()
            loss = fm_loss + args.lap_weight * lap_loss
        else:
        ...
        log_entry = {
            ...
            'lap_loss': lap_loss.item() if isinstance(lap_loss, torch.Tensor) else 0.0,
            'lr': lr
        }
