#!/usr/bin/env python3
"""
V5h: Multi-t Variance Reduction + Lower LR for Anti-Oscillation FM Training.

Root cause analysis: V5f (PerPointFinalLayer + FM) oscillates because random 
t-sampling creates high-variance gradients that push the DiT encoder between 
two basins (small-contour-optimized vs large-contour-optimized). EMA helps but 
eventually degrades when oscillation amplitude grows.

V5h addresses this with:
  1. Multi-t gradient accumulation (K=5): average FM loss over K different t values
     per optimizer step, reducing gradient variance by K×
  2. Lower LR (2e-4): smaller parameter updates reduce oscillation amplitude
  3. Gradient clipping (1.0): prevent large parameter jumps (inherited from V5f)

Architecture: same as V5f (PerPointFinalLayer + per-contour norm + FM)

Usage:
    CUDA_VISIBLE_DEVICES=1 FM_EPOCHS=10000 FM_SAVE_DIR=data/outputs/v37v5h_multit \
        python -u scripts/train_v37_gen_v5h.py
"""
...
def main():
    class Args:
        ...
        save_dir = os.environ.get('FM_SAVE_DIR', 'data/outputs/v37v5h_multit')
        ...
        lap_weight = float(os.environ.get('FM_LAP_WEIGHT', '0.0'))
        ...
        k_t = int(os.environ.get('FM_K_T', '5'))  # multi-t averaging
    args = Args()
    ...
    # Use PerPointFinalLayer instead of RegularizedPerPointFinalLayer
    cfg.v3_7_use_regularized_per_point = False
    os.makedirs(os.path.join(args.save_dir, 'checkpoints'), exist_ok=True)

    print(f"=== V3.7-gen V5h (Multi-t Variance Reduction + Lower LR) ===")
    ...
    print(f"  Multi-t K: {args.k_t} (gradient accumulation over K t-values)")
    print(f"  Zero x_0 probability: {args.zero_x0_prob}")
    print(f"  t ~ Beta({args.t_beta_alpha}, 1.0)")
    print(f"  Laplacian weight: {args.lap_weight}")
    ...
    print(f"\n[*] Starting V5h multi-t FM training ({args.epochs} epochs, K={args.k_t})...\n")
    t_start = time.time()
    ...
    for epoch in range(args.epochs):
        ...
        optimizer.zero_grad()
        total_fm_loss_val = 0.0

        for k in range(args.k_t):
            ...
            fm_loss_k = F.mse_loss(v_pred, v_target) / args.k_t
            fm_loss_k.backward()
            total_fm_loss_val += fm_loss_k.item() * args.k_t

        fm_loss_val = total_fm_loss_val / args.k_t

        # Gradient clipping and optimizer step (backward already done above)
        ...
        if (epoch + 1) % args.eval_every == 0 or epoch == 0:
            ...
            if mean_iou_ema > best_ema_iou:
                ...
                torch.save({
                    ...
                    'loss': fm_loss_val, 'iou': mean_iou_ema,
            ...
            print(f"  Ep {epoch:5d} | fm={fm_loss_val:.4e} | "
                  f"lr={lr:.2e} | IoU={mean_iou*100:.2f}% EMA={mean_iou_ema*100:.2f}% "
        ...
        if (epoch + 1) % args.save_every == 0:
            sd = {k: v.cpu() for k, v in model.state_dict().items()}
            torch.save({
                'state_dict': sd, 'epoch': epoch, 'loss': fm_loss_val,
                'raw_mode': True, 'contour_norm': True, 'per_point_head': True,
