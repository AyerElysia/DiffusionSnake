#!/usr/bin/env python3
"""
V4 of FM training for V3.7-gen denoiser.
Critical fix from V3: operate in RAW displacement space (no normalize/denormalize)
and use correct IoU evaluation against true ground truth.

Key changes:
  1. RAW MODE: FM operates directly in raw displacement space
     - x1 = x1_raw (no normalization)
     - ODE output is raw displacement (no denormalization)
     - Eliminates normalization-induced error amplification
  2. Correct IoU: compare against gt_polys_img (true GT), not normalized x1
  3. Noise scale proportional to data range (not fixed 0.1)
  4. EMA for stable evaluation
  5. Mixed x0 option (for regularization)

Usage:
    CUDA_VISIBLE_DEVICES=0 FM_EPOCHS=15000 FM_SAVE_DIR=data/outputs/v37v4a \
        python -u scripts/train_v37_gen_v4.py
"""
...
def ode_inference_raw(denoiser, cnn_feature, i_it_py, c_it_py, py_ind,
                       sampled_feat, h, w, steps=10, noise_scale=1.0,
                       deterministic=True):
    """ODE inference in RAW displacement space (no normalize/denormalize)."""
    ...
    for i in range(steps):
        ...
        x_t = x_t + v_pred * dt
    return x_t  # raw displacement, no denormalization needed
...
def main():
    ...
    print(f"=== V3.7-gen FM Training V4 (RAW Mode) ===")
    ...
    print(f"  Mode: RAW (no normalize/denormalize)")
    ...
    print(f"\n[*] Starting V4 RAW FM training ({args.epochs} epochs)...\n")
    t_start = time.time()
