#!/usr/bin/env python3
"""
V5c of FM training for V3.7-gen denoiser.
Same as V5b (per-contour normalization) but with:
  - delta_scale=1.0 (full per-point capacity, no artificial bottleneck)
  - t ~ Beta(0.3, 1.0) (more aggressive low-t bias)
  - Higher LR for faster convergence

Usage:
    CUDA_VISIBLE_DEVICES=0 FM_EPOCHS=15000 FM_SAVE_DIR=data/outputs/v37v5c \
        python -u scripts/train_v37_gen_v5c.py
"""
...
def main():
    class Args:
        ...
        t_beta_alpha = float(os.environ.get('FM_T_BETA_ALPHA', '0.3'))  # More aggressive low-t bias
    args = Args()
    ...
    print(f"=== V3.7-gen FM Training V5c (Per-Contour Norm + Full Delta) ===")
    ...
    # V5c: Override delta_scale to 1.0 for full per-point capacity
    if hasattr(denoiser, 'final_layer') and hasattr(denoiser.final_layer, 'delta_scale'):
        denoiser.final_layer.delta_scale = 1.0
        print(f"  *** delta_scale overridden to 1.0 ***")
