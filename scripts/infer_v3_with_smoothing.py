...
# 先解析与该脚本相关的参数，避免干扰 lib.config 自己的 argv 解析
...
_parser.add_argument('--curvature_threshold', default=1.8, type=float)
_parser.add_argument('--iterations', default=2, type=int)
...
def _repair_outliers(contour, z_thresh=2.0, neighbor_window=2):
    """先把明显飞点压回到局部邻域，避免平滑把长刺保留下来。"""
    ...
    outlier_mask = robust_z > float(z_thresh)

    if not outlier_mask.any():
        return contour
    if outlier_mask.mean() > 0.5:
        return contour
    ...
    for idx in np.where(outlier_mask)[0]:
        ...
        for step in range(1, int(neighbor_window) + 1):
            for sign in (-1, 1):
                j = (idx + sign * step) % n
                if not outlier_mask[j]:
                    neigh_pts.append(contour[j])
        if len(neigh_pts) == 0:
            continue
        neigh_pts = np.asarray(neigh_pts, dtype=np.float32)
        repaired[idx] = np.median(neigh_pts, axis=0)
    return repaired


def postprocess_contour(contour, outlier_z=2.0, curvature_threshold=1.8, iterations=2):
    contour = _repair_outliers(contour, z_thresh=outlier_z, neighbor_window=2)
    contour = smooth_contours_numpy(
        contour,
...
def load_v3_model(cfg_file=None, ckpt_path=None):
    if cfg_file:
    ...
    # 获取权重路径
    ckpt_path = ckpt_path or getattr(args, 'ckpt', '') or _custom_args.ckpt or os.path.join(
        _THIS_DIR,
        'data/outputs',
...
def run_inference(model, device, batch, save_dir, ver_tag, index,
                  outlier_z=2.0, curvature_threshold=1.8, iterations=2):
    ...
    with torch.no_grad():
        ...
        # ========== 新增：应用边缘感知平滑 ==========
        print(f"  [Smoothing] Applying outlier repair + edge-aware smoothing...")
        pred_polys_smoothed = []
        for poly in pred_polys:
            smoothed = postprocess_contour(
                poly,
                outlier_z=outlier_z,
                curvature_threshold=curvature_threshold,
                iterations=iterations,
            )
            pred_polys_smoothed.append(smoothed)
        pred_polys_smoothed = np.array(pred_polys_smoothed)
        print(f"  [Smoothing] Done. Smoothed {len(pred_polys_smoothed)} contours.")
        # ==========================================
    ...
    for poly in pred_polys:
        cv2.polylines(img, [poly.astype(np.int32)], True, (255, 0, 0), 1)  # Pred Original: 蓝（细线）
    for poly in pred_polys_smoothed:
        cv2.polylines(img, [poly.astype(np.int32)], True, (0, 0, 255), 2)  # Pred Smoothed: 红（粗线）

    save_path = os.path.join(save_dir, f"SMOOTHED_{ver_tag}_idx{index}_{datetime.datetime.now().strftime('%H%M%S')}.png")
    cv2.imwrite(save_path, img)
    print(f"[*] Saved: {save_path}")

def main():
    tag = _custom_args.tag or os.environ.get('TAG', 'v3')
    index = int(_custom_args.index if _custom_args.index is not None else os.environ.get('INDEX', 0))
    ...
    model, device = load_v3_model(cfg_file, ckpt_path=ckpt_path)
    dataset = make_dataset(cfg, cfg.test.dataset, make_transforms(cfg, False), False)
    collator = make_collator(cfg)

    save_dir = _custom_args.out_dir or os.path.join(_THIS_DIR, 'visual', 'v3_clean_eval')
    os.makedirs(save_dir, exist_ok=True)

    print(f"[*] Starting CLEAN inference (Tag: {tag}, Index: {index})...")
    print(f"[*] Postprocess: outlier_z={_custom_args.outlier_z}, curvature_threshold={_custom_args.curvature_threshold}, iterations={_custom_args.iterations}")
    ...
    run_inference(
        ...
        tag,
        index,
        outlier_z=_custom_args.outlier_z,
        curvature_threshold=_custom_args.curvature_threshold,
        iterations=_custom_args.iterations,
    )
