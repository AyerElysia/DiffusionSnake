...
LINEA_DIR=data/outputs/1232_final_diffusion_dit_v4_6c_geom_bridge_lineC_moonvit_jitter_gpu1
WATCH_LOG=test/lineC_ep15_eval_watcher.log
TARGET_EP=${1:-15}
        ...
        if [ -f "$CKPT" ]; then
            ...
            for i in $(seq 1 80); do
                if grep -q "mean_iou_sample_avg\|Saved summary" "$EVAL_LOG" 2>/dev/null; then
                    log "=== EVAL DONE ==="
                    grep -iE "mean_iou|mean_dice|Saved summary" "$EVAL_LOG" | tee -a test/lineC_pureFM_eval_result.log
                    break
                fi
                if ! ps -p $EPID > /dev/null 2>&1; then
                    log "eval process exited"
                    tail -15 "$EVAL_LOG" | tee -a test/lineC_pureFM_eval_result.log
                    break
                fi
                sleep 30
            done
