#!/bin/bash
# Phoenix watcher for Line A: auto-restart on death when GPU0 is free.
...
CFG_A=configs/1232_final_diffusion_dit_v4_6c_geom_bridge_lineC_moonvit_jitter_gpu1.yaml
LINEC_DIR=data/outputs/1232_final_diffusion_dit_v4_6c_geom_bridge_lineC_moonvit_jitter_gpu1
PHOENIX_LOG=test/phoenix_lineC.log
INTERVAL=${1:-180}
GPU=${2:-0}
...
launch() {
    local LOG_A="$LINEC_DIR/train_pureFM_phoenix_$(date +%Y%m%d_%H%M).log"
    ...
    local pid=$!
    log "LAUNCHED Line A PID=$pid GPU$GPU log=$LOG_A"
    sleep 45
    if lineC_alive; then
...
log "Will auto-restart Line A (pure FM, bs3+accum3) whenever it dies AND GPU$GPU has >=20GB free."

while true; do
    if lineC_alive; then
        : # healthy, do nothing
    else
        LAST_EP=$(tail -1 "$LINEC_DIR/logs.jsonl" 2>/dev/null | python3 -c "import sys,json
try: d=json.loads(sys.stdin.read()); print(d.get('epoch',-1))
except: print(-1)" 2>/dev/null)
        if gpu_free 20000; then
            log "Line A DEAD (last ep=$LAST_EP). GPU$GPU free. Restarting..."
            launch
        else
            log "Line A DEAD (last ep=$LAST_EP) but GPU$GPU not free, waiting..."
        fi
    fi
    sleep "$INTERVAL"
done
