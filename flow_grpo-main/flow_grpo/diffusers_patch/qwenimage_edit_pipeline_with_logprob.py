"""
QwenImage 图像编辑管线（带对数概率与窗口化记录）。

要点
- 同步编码图像与文本提示，支持负提示并采用 True-CFG 组合；
- 将编辑图像的 latent 拼接到 transformer 输入，预测后仅保留与生成 latent 对应的部分；
- 通过 sde_window 控制参与训练/统计的时间步范围，避免最后一步带来的数值溢出风险；
- 返回图像及中间 latent、log-prob、timesteps 以及相关嵌入与 image_latents。
"""
