"""
Qwen2-VL 图像生成管线（带对数概率与窗口化记录）。

改动亮点
- 使用与 SD3/FLUX 类似的 `sde_step_with_logprob`，在每步去噪中累积 log-prob；
- 支持 True-CFG：先分别计算条件/负条件预测，再按比例线性组合并做范数重标定；
- 通过 sde_window 在训练/评估时避开最后一步的数值尖峰，提升稳定性。

返回结果包含：最终图像、窗口内的所有 latent、每步 log-prob、对应 timesteps 与文本嵌入等。
"""
...
def pipeline_with_logprob(
    ...
    """
    QwenImage 生成接口（带 log-prob）。

    - 对输入 prompt/negative_prompt 一次性编码并切分；
    - 采用 True-CFG 组合条件预测，随后进行范数重标定以稳定幅值；
    - 仅在 sde_window 指定区间内记录 latent 与 log-prob，避免末步数值不稳定；
    - 最终使用 VAE 解码得到图像，连同中间统计量一并返回。
    """
