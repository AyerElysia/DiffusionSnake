"""RL V4e: Flow-GRPO SDE logprob over stochastic ODE-to-SDE inner steps."""
...
def _to_vis_image(batch: Dict) -> np.ndarray:
    img_raw = None
    if 'orig_img' in batch:
        orig = batch['orig_img']
        if isinstance(orig, (list, tuple)) and len(orig) > 0:
            img_raw = orig[0]
        elif torch.is_tensor(orig) and orig.numel() > 0:
            img_raw = orig[0]
        elif orig is not None:
            img_raw = orig
    if img_raw is None:
        h = int(batch['inp'].shape[-2]) if 'inp' in batch else 512
        w = int(batch['inp'].shape[-1]) if 'inp' in batch else 512
...
def _save_overlay_visual(
    sample_dir: Path,
    batch: Dict,
    init_py: torch.Tensor,
    gt_py: torch.Tensor,
    pred_py: torch.Tensor,
    mean_iou: float,
    mean_dice: float,
    mean_mbf: float,
):
    sample_dir.mkdir(parents=True, exist_ok=True)
    vis = _to_vis_image(batch)
    scale = float(snake_config.down_ratio)
    init_polys = init_py.detach().cpu().numpy() * scale
    gt_polys = gt_py.detach().cpu().numpy() * scale
    pred_polys = pred_py.detach().cpu().numpy() * scale

    for poly in gt_polys:
        cv2.polylines(vis, [np.round(poly).astype(np.int32)], True, (0, 255, 0), 2)
    for poly in init_polys:
        cv2.polylines(vis, [np.round(poly).astype(np.int32)], True, (0, 255, 255), 1)
    for idx, poly in enumerate(pred_polys):
        ip = np.round(poly).astype(np.int32)
        cv2.polylines(vis, [ip], True, (0, 0, 255), 2)
        cx = int(np.mean(ip[:, 0]))
        cy = int(np.mean(ip[:, 1]))
        cv2.putText(vis, str(idx), (cx - 8, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)

    cv2.putText(vis, f'IoU {mean_iou:.4f}', (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
    cv2.putText(vis, f'Dice {mean_dice:.4f}', (10, 44), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
    cv2.putText(vis, f'mBF {mean_mbf:.4f}', (10, 66), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
    cv2.imwrite(str(sample_dir / 'overlay.png'), vis)
    with open(sample_dir / 'meta.json', 'w') as f:
        json.dump({
            'mean_iou': float(mean_iou),
            'mean_dice': float(mean_dice),
            'mean_mboundf': float(mean_mbf),
            'num_contours': int(pred_py.shape[0]),
        }, f, indent=2)
...
def _action_logprob(action: torch.Tensor, mean: torch.Tensor, std: float) -> torch.Tensor:
    std_t = mean.new_full((mean.size(0), 1, 1), max(float(std), 1e-6))
    var = std_t.pow(2).clamp_min(1e-12)
    lp = -((action.detach() - mean) ** 2) / (2.0 * var)
    lp = lp - torch.log(std_t) - 0.5 * math.log(2.0 * math.pi)
...
def _outer_sde_sample(
    ...
    steps: int,
    noise_scale: float,
    sde_action_std: float,
    sde_noise_level: float,
    sde_type: str,
    sde_step_mode: str,
) -> Dict:
    ret = flow.sample_with_logprob(
        cnn_feature,
        i_state,
        c_state,
        py_ind,
        ...
        action_std=sde_action_std,
        step_mode=sde_step_mode,
        noise_level=sde_noise_level,
        sde_type=sde_type,
    )
    raw_disp = flow.clamp_pred_disp(ret['disp'], i_state)
    action = raw_disp * float(frac)
    return {
        'action': action.detach(),
        'disp': action.detach(),
        'x_ts': [x.detach() for x in ret.get('x_ts', [])],
        'x_prevs': [x.detach() for x in ret.get('x_prevs', [])],
        'old_logs': [x.detach() for x in ret.get('log_probs', [])],
        'timesteps': [float(t.detach().item() if torch.is_tensor(t) else t) for t in ret.get('timesteps', [])],
        'step_indices': [int(t.detach().item() if torch.is_tensor(t) else t) for t in ret.get('step_indices', [])],
        'x_self_conds': [None if x is None else x.detach() for x in ret.get('x_self_conds', [])],
        'sampled_feat': ret['sampled_feat'].detach(),
        'detail_feat': None if ret.get('detail_feat') is None else ret['detail_feat'].detach(),
        'contour_scale': ret['contour_scale'].detach(),
    }


def main():
    print('=' * 72)
    print('[RL-V4e] Three outer refinements with Flow-GRPO SDE logprob')
    print('=' * 72)
    ...
    sde_action_std = float(cv('sde_action_std', 1.0))
    sde_noise_level = float(cv('sde_noise_level', 0.7))
    sde_type = str(cv('sde_type', 'sde')).strip().lower()
    sde_step_mode = str(cv('sde_step_mode', 'gaussian')).strip().lower()
    eval_seed_stride = int(cv('eval_seed_stride', 1000003))
    ...
    for group in optimizer.param_groups:
        group['lr'] = lr
    print(f'[RL-V4e] optimizer LR set to {lr}')

    ref_flow = freeze_ref_flow(inner)
    print('[RL-V4e] frozen reference flow snapshot created.')
    ...
    with open(log_dir / 'v4_hparams.json', 'w') as f:
        json.dump({
            ...
            'ode_steps': ode_steps,
            'noise_scale': noise_scale,
            'sde_action_std': sde_action_std,
            'sde_noise_level': sde_noise_level,
            'sde_type': sde_type,
            'sde_step_mode': sde_step_mode,
        ...
        if resume_step > 0:
            ...
            if isinstance(opt_state, dict):
                try:
                    ...
                    for group in optimizer.param_groups:
                        group['lr'] = lr
                    print(f'[RL-V4] resumed optimizer/state from step {resume_step}')
                except Exception as e:
                    print(f'[RL-V4] WARNING: failed to resume optimizer state: {e}')
            print(f'[RL-V4] resume mode on: start_step={start_step} train_steps={train_steps}')
    ...
    @torch.no_grad()
    def _stochastic_sde_three_step(output):
        current = output['i_it_py'].detach()
        total_disp = torch.zeros_like(current)
        for frac in fractions:
            c_cur = snake_gcn_utils.img_poly_to_can_poly(current)
            ret = _outer_sde_sample(
                gcn,
                output['cnn_feature'],
                current,
                c_cur,
    ...
    def _sample_rollout(output):
        ...
        for frac in fractions:
            ...
            action = ret['action']
            for ai, old_log in enumerate(ret['old_logs']):
                traj['states'].append(current.detach())
                traj['c_states'].append(c_cur.detach())
                traj['x_ts'].append(ret['x_ts'][ai])
                traj['x_prevs'].append(ret['x_prevs'][ai])
                traj['x_self_conds'].append(ret['x_self_conds'][ai])
                traj['old_logs'].append(old_log)
                traj['fractions'].append(float(frac))
                traj['step_indices'].append(ret['step_indices'][ai])
                traj['timesteps'].append(ret['timesteps'][ai])
                traj['sampled_feats'].append(ret['sampled_feat'])
                traj['detail_feats'].append(ret['detail_feat'])
                traj['contour_scales'].append(ret['contour_scale'])
            current = (current + action).detach()
            total_disp = total_disp + action.detach()
    ...
    def _compute_eval():
        ...
        for bi, eb in enumerate(eval_batches):
            _set_seed(seed + bi * eval_seed_stride)
            out = _manual_context(eb)
            if out['i_it_py'].numel() == 0:
                continue
            det = _stochastic_sde_three_step(out)
    ...
    def _compute_full_eval():
        save_dir = _project_path(os.environ.get('SAVE_DIR', f'visual/{_cfg_stem()}_v4e_sde_eval'))
        ...
        max_visuals_env = os.environ.get('MAX_VISUALS', '').strip()
        max_visuals = int(max_visuals_env) if max_visuals_env else None
        per_sample_root = save_dir / 'per_sample'
        if save_visuals:
        ...
        for index, batch in enumerate(full_loader):
            if max_samples is not None and index >= max_samples:
                break
            try:
                _set_seed(seed + index * eval_seed_stride)
                _move_batch(batch)
                ...
                mbf = compute_region_score(pred, gt, H=hw[0], W=hw[1], w_boundary=1, w_dice=0, w_iou=0,
                                           coord_scale=float(snake_config.down_ratio))
                sample_dir = ''
                if save_visuals and (max_visuals is None or saved_visuals < max_visuals):
                    sample_dir = str(per_sample_root / f'idx_{index:03d}')
                    _save_overlay_visual(
                        Path(sample_dir),
                        batch,
                        out['i_it_py'],
                        gt,
        ...
        if vals_iou:
            ...
            sample_dice = [r['mean_dice'] for r in rows if r.get('ok')]
            sample_mbf = [r['mean_mboundf'] for r in rows if r.get('ok')]
            summary = {
                ...
                'eval_mode': 'stochastic_sde_fixed_seed',
                'eval_seed': int(seed),
                'eval_seed_stride': int(eval_seed_stride),
                'sde_noise_level': float(sde_noise_level),
                'sde_type': str(sde_type),
                'sde_step_mode': str(sde_step_mode),
                'save_visuals': bool(save_visuals),
                'saved_visuals': int(saved_visuals),
                'per_sample_root': str(per_sample_root) if save_visuals else '',
                'mean_iou_sample_avg': float(np.mean(sample_ious)) if sample_ious else 0.0,
                'mean_iou_contour_avg': float(iou_all.mean().item()),
                'mean_dice_sample_avg': float(np.mean(sample_dice)) if sample_dice else 0.0,
                'mean_dice_contour_avg': float(dice_all.mean().item()),
                'mean_mboundf_sample_avg': float(np.mean(sample_mbf)) if sample_mbf else 0.0,
        ...
        summary_path = save_dir / f'v4e_sde_full_eval_{ts}.json'
        rows_path = save_dir / f'v4e_sde_full_eval_rows_{ts}.json'
    ...
    if _as_bool(os.environ.get('RL_V4E_EVAL_ONLY', False)):
        _compute_full_eval()
        return
    ...
    for step in range(start_step, train_steps + 1):
        ...
        total_actions = sum(len(t['old_logs']) for t in rollouts)
            ...
            for ri, traj in enumerate(rollouts):
                ...
                for si, x_t in enumerate(traj['x_ts']):
                    ...
                    if kl_beta > 0:
                        ...
                        var = std_cur.detach().pow(2).clamp_min(1e-12)
                        kl_loss = ((mean_cur - mean_ref).pow(2) / (2.0 * var)).mean() / max(total_actions, 1)
        ...
        metrics = {
            ...
            'sde_log_count_mean': float(np.mean(old_log_counts)) if old_log_counts else 0.0,
        ...
        if step % log_every == 0:
            ...
            print(
                ...
                f"kl={metrics['approx_kl']:.6f} logs={metrics['sde_log_count_mean']:.1f}{extra}",
                flush=True,
            )
